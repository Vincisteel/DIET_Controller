...
class Logger:
    ...
    num_iterations:int):
        ...
        self.RESULT_PATH = f"{logging_path}/results/{str(date.year)}_{str(date.month)}_{str(date.day)}/results_{self.time}"
    ...
    def _plot(
        ...
        fig.add_trace(go.Scatter(name='Training Log Loss', x=t, y=losses.flatten(), mode='lines',
    ...
    def plot_pmv_percentages(self, pmv: np.ndarray, savepath:str, title:str, plot_title:str):
    ...
    def plot_and_logging(self, episode_num, tair, actions, pmv, qheat, rewards, occ, losses,epsilons,agent, is_summary=False):
        ...
        plot_title = f"Episode Number {suffix} with α = {agent.env.alpha} and β = {agent.env.beta}" \
            if not(is_summary) else f"Summary with α = {agent.env.alpha} and β = {agent.env.beta}"
        ...
        log_dict["pmvs"] = {pmv_df["intervals"][i]:pmv_df["ranges"][i] for i in pmv_df["intervals"].keys()}
